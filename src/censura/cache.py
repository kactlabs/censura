...
def get_cache_dir() -> Path:
    """Get the cache directory used by censura.

    Users can customize this directory on all systems using `CENSURA_CACHE_DIR`
    environment variable. By default, the cache directory is the user cache directory
    under the censura application.

    This result is immediately set to a constant `censura.cache.CACHE_DIR` as to avoid
    repeated calls.
    """
    # NOTE: Function mostly exists as a clean way to test getting the cache directory.
    default_cache_dir = user_cache_dir("censura")
    cache_dir = Path(os.environ.get("CENSURA_CACHE_DIR", default_cache_dir))
