...
from censura.output import out
from censura.report import NothingChanged
...
def jupyter_dependencies_are_installed(*, warn: bool) -> bool:
    ...
    if not installed and warn:
        msg = (
            ...
            'You can fix this by running ``pip install "censura[jupyter]"``'
        )
        out(msg)
    return installed
...
def get_token(src: str, magic: str) -> str:
    ...
    while token in src:
        ...
        if counter > 100:
            raise AssertionError(
                "INTERNAL ERROR: Censura was not able to replace IPython magic. "
                "Please report a bug on https://github.com/kactlabs/censura/issues.  "
...
class MagicFinder(ast.NodeVisitor):
    ...
    def visit_Assign(self, node: ast.Assign) -> None:
        ...
        For example,
            ...
            censura_version = !censura --version
            env = %env var

        would have been (respectively) transformed to

            censura_version = get_ipython().getoutput('censura --version')
