...
from censura.output import err
...
def find_project_root(
    ...
    pyproject.toml files are only considered if they contain a [tool.censura]
    ...
    for directory in (common_base, *common_base.parents):
        ...
        if (directory / "pyproject.toml").is_file():
            ...
            if "censura" in pyproject_toml.get("tool", {}):
...
def parse_pyproject_toml(path_config: str) -> dict[str, Any]:
    """Parse a pyproject toml file, pulling out relevant parts for Censura.

    If parsing fails, will raise a tomllib.TOMLDecodeError.
    """
    pyproject_toml = _load_toml(path_config)
    config: dict[str, Any] = pyproject_toml.get("tool", {}).get("censura", {})
...
def infer_target_version(
    ...
    """Infer Censura's target version from the project metadata in pyproject.toml.
...
def find_user_pyproject_toml() -> Path:
    r"""Return the path to the top-level user configuration for censura.

    This looks for ~\.censura on Windows and ~/.config/censura on Linux and other
    Unix systems.
    ...
    if sys.platform == "win32":
        ...
        user_config_path = Path(config_root).expanduser() / "censura"
