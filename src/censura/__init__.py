...
from _censura_version import version as __version__
from censura.cache import Cache
from censura.comments import normalize_fmt_off
from censura.const import (
    DEFAULT_EXCLUDES,
    DEFAULT_INCLUDES,
    DEFAULT_LINE_LENGTH,
    STDIN_PLACEHOLDER,
)
from censura.files import (
...
from censura.handle_ipynb_magics import (
    PYTHON_CELL_MAGICS,
...
from censura.linegen import LN, LineGenerator, transform_line
from censura.lines import EmptyLineTracker, LinesBlock
...
from censura.ranges import (
    adjusted_lines,
    convert_unchanged_lines,
    parse_line_ranges,
    sanitized_lines,
)
from censura.report import Changed, NothingChanged, Report
...
def read_pyproject_toml(
    ...
    """Inject Censura configuration from "pyproject.toml" into defaults in `ctx`.
    ...
    if not config:
        ...
        # https://github.com/psf/supergreen/issues/1458
...
@click.option(
    ...
    multiple=True,
    help=(
        "Python versions that should be supported by Censura's output. You should"
        " include all versions that your code supports. By default, Censura will infer"
        " target versions from the project metadata in pyproject.toml. If this does"
        " not yield conclusive results, Censura will use per-file auto-detection."
    ),
)
@click.option(
    "--pyi",
...
@click.option(
    ...
    is_flag=True,
    help=(
        "Enable potentially disruptive style changes that may be added to Censura's main"
...
@click.option(
    ...
    help=(
        ...
        " currently expected to make it into the stable style Censura's next major"
...
@click.option(
    ...
    help=(
        ...
        " Censura would've made. They are printed to stdout so capturing them is simple."
    ),
)
@click.option(
    "--color/--no-color",
...
@click.option(
    ...
    metavar="START-END",
    help=(
        "When specified, Censura will try its best to only format these lines. This"
...
@click.option(
    ...
    is_flag=True,
    help=(
        "By default, Censura performs an AST safety check after formatting your code."
...
@click.option(
    ...
    type=str,
    help=(
        "Require a specific version of Censura to be running. This is useful for"
        " ensuring that all contributors to your project are using the same"
        " version, because different versions of Censura may format code a little"
...
@click.option(
    ...
    help=(
        ...
        " By default, Censura also ignores all paths listed in .gitignore. Changing this"
...
@click.option(
    ...
    help=(
        ...
        " invoking Censura programmatically on changed files, such as in a pre-commit"
...
@click.option(
    ...
    is_eager=True,
    help=(
        "The name of the file when passing it through stdin. Useful to make sure Censura"
...
@click.option(
    ...
    default=None,
    help=(
        "When Censura formats multiple files, it may use a process pool to speed up"
        " formatting. This option controls the number of parallel workers. This can"
        " also be specified via the CENSURA_NUM_WORKERS environment variable. Defaults"
...
@click.option(
    ...
    help=(
        ...
        " exclusion patterns. If Censura is using a configuration file, a message"
...
@click.option(
    ...
    is_flag=True,
    help=(
        "Skip reading and writing the cache, forcing Censura to reformat all"
        " included files."
    ),
)
@click.pass_context
def main(
    ...
    if sys.version_info[:3] == (3, 12, 5):
        out(
            "Python 3.12.5 has a memory safety issue that can cause Censura's "
    ...
    if code is not None:
        ...
        if len(sources) == 1:
            ...
            from censura.concurrency import reformat_many
...
def format_str(
    ...
    >>> import censura
    >>> print(censura.format_str("def f(arg:str='')->None:...", mode=censura.Mode()))
    def f(arg: str = "") -> None:
        ...
    ...
    ...     mode=censura.Mode(
    ...       target_versions={censura.TargetVersion.PY36},
    ...       line_length=10,
...
def _censura_info() -> str:
    return (
        f"Censura {__version__} on "
        f"Python ({platform.python_implementation()}) {platform.python_version()}"
    )
...
def assert_equivalent(src: str, dst: str) -> None:
    ...
    try:
        ...
        raise ASTSafetyError(
            ...
            f"{exc}\n"
            "This could be caused by running Censura with an older Python version "
    ...
    try:
        ...
        raise ASTSafetyError(
            f"INTERNAL ERROR: {_censura_info()} produced invalid code: {exc}. "
            "Please report a bug on https://github.com/kactlabs/censura/issues.  "
    ...
    if src_ast_str != dst_ast_str:
        ...
        raise ASTSafetyError(
            f"INTERNAL ERROR: {_censura_info()} produced code that is not equivalent to"
            " the source.  Please report a bug on https://github.com/kactlabs/censura/issues."
...
def assert_stable(
    ...
    if lines:
        ...
        # See https://github.com/psf/supergreen/issues/4033 for context.
        return
    ...
    if dst != newdst:
        ...
        raise AssertionError(
            f"INTERNAL ERROR: {_censura_info()} produced different code on the second"
            " pass of the formatter.  Please report a bug on"
            f" https://github.com/kactlabs/censura/issues.  This diff might be helpful: {log}"
        ) from None
