...
try:
    ...
    raise ImportError(
        ...
        + "Please re-install censura with the '[d]' extra install "
        + "to obtain aiohttp_cors: `pip install censura[d]`"
    ) from None
...
from censura.concurrency import maybe_install_uvloop
...
# Response headers
CENSURA_VERSION_HEADER = "X-Censura-Version"
...
@click.version_option(version=censura.__version__)
def main(bind_host: str, bind_port: int) -> None:
    logging.basicConfig(level=logging.INFO)
    app = make_app()
    ver = censura.__version__
    censura.out(f"censurad version {ver} listening on {bind_host} port {bind_port}")
...
async def handle(request: web.Request, executor: Executor) -> web.Response:
    ...
    try:
        ...
        formatted_str = await loop.run_in_executor(
            executor, partial(censura.format_file_contents, req_str, fast=fast, mode=mode)
        )
        ...
        if only_diff:
            ...
            formatted_str = await loop.run_in_executor(
                executor,
                partial(censura.diff, req_str, formatted_str, src_name, dst_name),
            )
    ...
    except censura.InvalidInput as e:
...
def parse_mode(headers: MultiMapping[str]) -> censura.Mode:
    try:
        line_length = int(headers.get(LINE_LENGTH_HEADER, censura.DEFAULT_LINE_LENGTH))
    except ValueError:
...
def parse_python_variant_header(value: str) -> tuple[bool, set[censura.TargetVersion]]:
    if value == "pyi":
        return True, set()
    else:
        versions = set()
        for version in value.split(","):
            ...
            try:
                ...
                if major == 3 and not hasattr(censura.TargetVersion, version_str):
                    raise InvalidVariantHeader(f"3.{minor} is not supported")
                versions.add(censura.TargetVersion[version_str])
