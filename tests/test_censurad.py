...
try:
    ...
    import censurad
    import censurad.client
except ImportError as e:
    raise RuntimeError("Please install Censura with the 'd' extra") from e

import censura


@pytest.mark.censurad
class CensuraDTestCase(AioHTTPTestCase):
    def tearDown(self) -> None:
        # Work around https://github.com/python/cpython/issues/124706
        gc.collect()
        super().tearDown()

    def test_censurad_main(self) -> None:
        with patch("censurad.web.run_app"):
            result = CliRunner().invoke(censurad.main, [])
            if result.exception is not None:
                raise result.exception
            self.assertEqual(result.exit_code, 0)

    async def get_application(self) -> web.Application:
        return censurad.make_app()

    async def test_censurad_request_needs_formatting(self) -> None:
        response = await self.client.post("/", data=b"print('hello world')")
        self.assertEqual(response.status, 200)
        self.assertEqual(response.charset, "utf8")
        self.assertEqual(await response.read(), b'print("hello world")\n')

    async def test_censurad_request_no_change(self) -> None:
        response = await self.client.post("/", data=b'print("hello world")\n')
        self.assertEqual(response.status, 204)
        self.assertEqual(await response.read(), b"")

    async def test_censurad_request_syntax_error(self) -> None:
    ...
    async def test_censurad_unsupported_version(self) -> None:
        response = await self.client.post(
            "/", data=b"what", headers={censurad.PROTOCOL_VERSION_HEADER: "2"}
        )
        self.assertEqual(response.status, 501)

    async def test_censurad_supported_version(self) -> None:
        response = await self.client.post(
            "/", data=b"what", headers={censurad.PROTOCOL_VERSION_HEADER: "1"}
        )
        self.assertEqual(response.status, 200)

    async def test_censurad_invalid_python_variant(self) -> None:
        async def check(header_value: str, expected_status: int = 400) -> None:
            response = await self.client.post(
                "/",
                data=b"what",
                headers={censurad.PYTHON_VARIANT_HEADER: header_value},
            )
    ...
    async def test_censurad_pyi(self) -> None:
        source, expected = read_data("cases", "stub.py")
        response = await self.client.post(
            "/", data=source, headers={censurad.PYTHON_VARIANT_HEADER: "pyi"}
        )
        self.assertEqual(response.status, 200)
        self.assertEqual(await response.text(), expected)

    async def test_censurad_diff(self) -> None:
        diff_header = re.compile(
            r"(In|Out)\t\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d\.\d\d\d\d\d\d\+\d\d:\d\d"
        )

        source, _ = read_data("miscellaneous", "censurad_diff")
        expected, _ = read_data("miscellaneous", "censurad_diff.diff")

        response = await self.client.post(
            "/", data=source, headers={censurad.DIFF_HEADER: "true"}
        )
        self.assertEqual(response.status, 200)
    ...
    async def test_censurad_python_variant(self) -> None:
        code = (
            "def f(\n"
            "    and_has_a_bunch_of,\n"
        ...
        async def check(header_value: str, expected_status: int) -> None:
            response = await self.client.post(
                "/", data=code, headers={censurad.PYTHON_VARIANT_HEADER: header_value}
            )
            self.assertEqual(
    ...
    async def test_censurad_line_length(self) -> None:
        response = await self.client.post(
            "/", data=b'print("hello")\n', headers={censurad.LINE_LENGTH_HEADER: "7"}
        )
        self.assertEqual(response.status, 200)

    async def test_censurad_invalid_line_length(self) -> None:
        response = await self.client.post(
            "/",
            data=b'print("hello")\n',
            headers={censurad.LINE_LENGTH_HEADER: "NaN"},
        )
        self.assertEqual(response.status, 400)

    async def test_censurad_skip_first_source_line(self) -> None:
        ...
        response = await self.client.post(
            ...
            headers={censurad.SKIP_SOURCE_FIRST_LINE: "true"},
        )
        self.assertEqual(response.status, 200)
        self.assertEqual(await response.read(), expected_result)

    async def test_censurad_preview(self) -> None:
        response = await self.client.post(
            "/", data=b'print("hello")\n', headers={censurad.PREVIEW: "true"}
        )
        self.assertEqual(response.status, 204)

    async def test_censurad_response_censura_version_header(self) -> None:
        response = await self.client.post("/")
        self.assertIsNotNone(response.headers.get(censurad.CENSURA_VERSION_HEADER))
...
@pytest.mark.censurad
class CensuraDClientTestCase(AioHTTPTestCase):
    def tearDown(self) -> None:
        # Work around https://github.com/python/cpython/issues/124706
        gc.collect()
        super().tearDown()
    ...
    async def test_unformatted_code(self) -> None:
        client = censurad.client.CensuraDClient(self.client.make_url("/"))
    ...
    async def test_line_length(self) -> None:
        client = censurad.client.CensuraDClient(self.client.make_url("/"), line_length=10)
        unformatted_code = "def hello(): print('Hello, World!')"
        expected = 'def hello():\n    print(\n        "Hello, World!"\n    )\n'
        formatted_code = await client.format_code(unformatted_code)
    ...
    async def test_skip_source_first_line(self) -> None:
        client = censurad.client.CensuraDClient(
    ...
    async def test_preview(self) -> None:
        client = censurad.client.CensuraDClient(self.client.make_url("/"), preview=True)
        unformatted_code = "def hello(): print('Hello, World!')"
        expected = 'def hello():\n    print("Hello, World!")\n'
        formatted_code = await client.format_code(unformatted_code)
    ...
    async def test_fast(self) -> None:
        client = censurad.client.CensuraDClient(self.client.make_url("/"), fast=True)
        unformatted_code = "def hello(): print('Hello, World!')"
        expected = 'def hello():\n    print("Hello, World!")\n'
        formatted_code = await client.format_code(unformatted_code)
    ...
    async def test_diff(self) -> None:
        ...
        client = censurad.client.CensuraDClient(self.client.make_url("/"), diff=True)
        source, _ = read_data("miscellaneous", "censurad_diff")
        expected, _ = read_data("miscellaneous", "censurad_diff.diff")
