...
import censura
import censura.files
from censura import Feature, TargetVersion
from censura import re_compile_maybe_verbose as compile_pattern
from censura.cache import FileData, get_cache_dir, get_cache_file
from censura.debug import DebugVisitor
from censura.mode import Mode, Preview
from censura.output import color_diff, diff
from censura.parsing import ASTSafetyError
from censura.report import Report
from censura.strings import lines_with_leading_tabs_expanded
...
DEFAULT_EXCLUDE = censura.re_compile_maybe_verbose(censura.const.DEFAULT_EXCLUDES)
DEFAULT_INCLUDE = censura.re_compile_maybe_verbose(censura.const.DEFAULT_INCLUDES)
T = TypeVar("T")
R = TypeVar("R")
...
def cache_dir(exists: bool = True) -> Iterator[Path]:
    with TemporaryDirectory() as workspace:
        ...
        with patch("censura.cache.CACHE_DIR", cache_dir):
            yield cache_dir
...
class FakeContext(click.Context):
    ...
    def __init__(self) -> None:
        ...
        self.command: click.Command = censura.main
...
class CensuraRunner(CliRunner):
    """Make sure STDOUT and STDERR are kept separate when testing Censura via its CLI."""
...
def invokeCensura(
    ...
    result = runner.invoke(censura.main, args, catch_exceptions=False)
...
class CensuraTestCase(CensuraBaseTestCase):
    ...
    def test_empty_ff(self) -> None:
        expected = ""
        tmp_file = Path(censura.dump_to_file())
        try:
            self.assertFalse(ff(tmp_file, write_back=censura.WriteBack.YES))
            actual = tmp_file.read_text(encoding="utf-8")
        finally:
            os.unlink(tmp_file)
        self.assertFormatEqual(expected, actual)

    @patch("censura.dump_to_file", dump_to_stderr)
    ...
    def test_one_empty_line_ff(self) -> None:
        for nl in ["\n", "\r\n"]:
            expected = nl
            tmp_file = Path(censura.dump_to_file(nl))
            if system() == "Windows":
                ...
                # https://github.com/psf/supergreen/pull/3348
    ...
    def test_piping(self) -> None:
        _, source, expected = read_data_from_file(
            PROJECT_ROOT / "src/censura/__init__.py"
        )
        result = CensuraRunner().invoke(
            ...
            [
                "-",
                "--fast",
                f"--line-length={censura.DEFAULT_LINE_LENGTH}",
        ...
        if source != result.stdout:
            censura.assert_equivalent(source, result.stdout)
            censura.assert_stable(source, result.stdout, DEFAULT_MODE)
    ...
    def test_piping_diff(self) -> None:
        ...
        result = CensuraRunner().invoke(
            censura.main, args, input=BytesIO(source.encode("utf-8"))
        )
        self.assertEqual(result.exit_code, 0)
    ...
    def test_pep_572_version_detection(self) -> None:
        ...
        root = censura.lib2to3_parse(source)
        features = censura.get_features_used(root)
        self.assertIn(censura.Feature.ASSIGNMENT_EXPRESSIONS, features)
        versions = censura.detect_target_versions(root)
        self.assertIn(censura.TargetVersion.PY38, versions)
    ...
    def test_pep_695_version_detection(self) -> None:
        for file in ("type_aliases", "type_params"):
            ...
            root = censura.lib2to3_parse(source)
            features = censura.get_features_used(root)
            self.assertIn(censura.Feature.TYPE_PARAMS, features)
            versions = censura.detect_target_versions(root)
            self.assertIn(censura.TargetVersion.PY312, versions)
    ...
    def test_pep_696_version_detection(self) -> None:
        ...
        for sample in samples:
            root = censura.lib2to3_parse(sample)
            features = censura.get_features_used(root)
            self.assertIn(censura.Feature.TYPE_PARAM_DEFAULTS, features)

    def test_expression_ff(self) -> None:
        source, expected = read_data("cases", "expression.py")
        tmp_file = Path(censura.dump_to_file(source))
        try:
            self.assertTrue(ff(tmp_file, write_back=censura.WriteBack.YES))
            actual = tmp_file.read_text(encoding="utf-8")
        finally:
            os.unlink(tmp_file)
        self.assertFormatEqual(expected, actual)
        with patch("censura.dump_to_file", dump_to_stderr):
    ...
    def test_expression_diff(self) -> None:
        ...
        try:
            result = CensuraRunner().invoke(
                censura.main, ["--diff", str(tmp_file), f"--config={EMPTY_CONFIG}"]
            )
            self.assertEqual(result.exit_code, 0)
        finally:
            os.unlink(tmp_file)
        ...
        if expected != actual:
            dump = censura.dump_to_file(actual)
            msg = (
    ...
    def test_detect_pos_only_arguments(self) -> None:
        ...
        root = censura.lib2to3_parse(source)
        features = censura.get_features_used(root)
        self.assertIn(censura.Feature.POS_ONLY_ARGUMENTS, features)
        versions = censura.detect_target_versions(root)
        self.assertIn(censura.TargetVersion.PY38, versions)

    def test_detect_debug_f_strings(self) -> None:
        root = censura.lib2to3_parse("""f"{x=}" """)
        features = censura.get_features_used(root)
        self.assertIn(censura.Feature.DEBUG_F_STRINGS, features)
        versions = censura.detect_target_versions(root)
        self.assertIn(censura.TargetVersion.PY38, versions)

        root = censura.lib2to3_parse(
            """f"{x}"\nf'{"="}'\nf'{(x:=5)}'\nf'{f(a="3=")}'\nf'{x:=10}'\n"""
        )
        features = censura.get_features_used(root)
        self.assertNotIn(censura.Feature.DEBUG_F_STRINGS, features)
    ...
    def test_string_quotes(self) -> None:
        ...
        mode = censura.Mode(unstable=True)
        assert_format(source, expected, mode)
        mode = replace(mode, string_normalization=False)
        not_normalized = fs(source, mode=mode)
        self.assertFormatEqual(source.replace("\\\n", ""), not_normalized)
        censura.assert_equivalent(source, not_normalized)
        censura.assert_stable(source, not_normalized, mode=mode)
    ...
    def test_skip_source_first_line_when_mixing_newlines(self) -> None:
        ...
        with TemporaryDirectory() as workspace:
            ...
            ff(test_file, mode=mode, write_back=censura.WriteBack.YES)
    ...
    def test_skip_magic_trailing_comma(self) -> None:
        ...
        try:
            result = CensuraRunner().invoke(
                censura.main, ["-C", "--diff", str(tmp_file), f"--config={EMPTY_CONFIG}"]
            )
            self.assertEqual(result.exit_code, 0)
        finally:
            os.unlink(tmp_file)
    ...
    def test_false_positive_symlink_output_issue_3384(self) -> None:
        # Emulate the behavior when using the CLI (`censura ./child  --verbose`), which
    ...
    def test_report_verbose(self) -> None:
        ...
        with patch("censura.output._out", out), patch("censura.output._err", err):
            report.done(Path("f1"), censura.Changed.NO)
            self.assertEqual(len(out_lines), 1)
            self.assertEqual(len(err_lines), 0)
            self.assertEqual(out_lines[-1], "f1 already well formatted, good job.")
            self.assertEqual(unstyle(str(report)), "1 file left unchanged.")
            self.assertEqual(report.return_code, 0)
            report.done(Path("f2"), censura.Changed.YES)
            ...
            report.done(Path("f3"), censura.Changed.CACHED)
            ...
            report.done(Path("f3"), censura.Changed.YES)
            ...
            self.assertEqual(report.return_code, 123)
            report.done(Path("f4"), censura.Changed.NO)
    ...
    def test_lib2to3_parse(self) -> None:
        with self.assertRaises(censura.InvalidInput):
            censura.lib2to3_parse("invalid syntax")

        straddling = "x + y"
        censura.lib2to3_parse(straddling)
        censura.lib2to3_parse(straddling, {TargetVersion.PY36})

        py2_only = "print x"
        with self.assertRaises(censura.InvalidInput):
            censura.lib2to3_parse(py2_only, {TargetVersion.PY36})

        py3_only = "exec(x, end=y)"
        censura.lib2to3_parse(py3_only)
        censura.lib2to3_parse(py3_only, {TargetVersion.PY36})
    ...
    def test_get_features_used_decorator(self) -> None:
        ...
        for simple_test in simples.split("##")[1:]:
            node = censura.lib2to3_parse(simple_test)
            decorator = str(node.children[0].children[0]).strip()
            self.assertNotIn(
                Feature.RELAXED_DECORATORS,
                censura.get_features_used(node),
                msg=(
        ...
        for relaxed_test in relaxed.split("##")[1:]:
            node = censura.lib2to3_parse(relaxed_test)
            decorator = str(node.children[0].children[0]).strip()
            self.assertIn(
                Feature.RELAXED_DECORATORS,
                censura.get_features_used(node),
    ...
    def check_features_used(self, source: str, expected: set[Feature]) -> None:
        node = censura.lib2to3_parse(source)
        actual = censura.get_features_used(node)
    ...
    def test_get_features_used_for_future_flags(self) -> None:
        for src, features in [
            ...
            with self.subTest(src=src, features=sorted(f.value for f in features)):
                node = censura.lib2to3_parse(src)
                future_imports = censura.get_future_imports(node)
                self.assertEqual(
                    censura.get_features_used(node, future_imports=future_imports),
                    features,
                )

    def test_get_future_imports(self) -> None:
        node = censura.lib2to3_parse("\n")
        self.assertEqual(set(), censura.get_future_imports(node))
        node = censura.lib2to3_parse("from __future__ import censura\n")
        self.assertEqual({"censura"}, censura.get_future_imports(node))
        node = censura.lib2to3_parse("from __future__ import multiple, imports\n")
        self.assertEqual({"multiple", "imports"}, censura.get_future_imports(node))
        node = censura.lib2to3_parse("from __future__ import (parenthesized, imports)\n")
        self.assertEqual({"parenthesized", "imports"}, censura.get_future_imports(node))
        node = censura.lib2to3_parse(
            "from __future__ import multiple\nfrom __future__ import imports\n"
        )
        self.assertEqual({"multiple", "imports"}, censura.get_future_imports(node))
        node = censura.lib2to3_parse("# comment\nfrom __future__ import censura\n")
        self.assertEqual({"censura"}, censura.get_future_imports(node))
        node = censura.lib2to3_parse('"""docstring"""\nfrom __future__ import censura\n')
        self.assertEqual({"censura"}, censura.get_future_imports(node))
        node = censura.lib2to3_parse("some(other, code)\nfrom __future__ import censura\n")
        self.assertEqual(set(), censura.get_future_imports(node))
        node = censura.lib2to3_parse("from some.module import censura\n")
        self.assertEqual(set(), censura.get_future_imports(node))
        node = censura.lib2to3_parse(
            "from __future__ import unicode_literals as _unicode_literals"
        )
        self.assertEqual({"unicode_literals"}, censura.get_future_imports(node))
        node = censura.lib2to3_parse(
            "from __future__ import unicode_literals as _lol, print"
        )
        self.assertEqual({"unicode_literals", "print"}, censura.get_future_imports(node))
    ...
    def test_debug_visitor(self) -> None:
        ...
        with patch("censura.debug.out", out):
            DebugVisitor.show(source)
        actual = "\n".join(out_lines) + "\n"
        log_name = ""
        if expected != actual:
            log_name = censura.dump_to_file(*out_lines)
        self.assertEqual(
            expected,
            actual,
    ...
    def test_format_file_contents(self) -> None:
        mode = DEFAULT_MODE
        empty = ""
        with self.assertRaises(censura.NothingChanged):
            censura.format_file_contents(empty, mode=mode, fast=False)
        just_nl = "\n"
        with self.assertRaises(censura.NothingChanged):
            censura.format_file_contents(just_nl, mode=mode, fast=False)
        same = "j = [1, 2, 3]\n"
        with self.assertRaises(censura.NothingChanged):
            censura.format_file_contents(same, mode=mode, fast=False)
        different = "j = [1,2,3]"
        expected = same
        actual = censura.format_file_contents(different, mode=mode, fast=False)
        self.assertEqual(expected, actual)
        invalid = "return if you can"
        with self.assertRaises(censura.InvalidInput) as e:
            censura.format_file_contents(invalid, mode=mode, fast=False)
        self.assertEqual(str(e.exception), "Cannot parse: 1:7: return if you can")

        just_crlf = "\r\n"
        with self.assertRaises(censura.NothingChanged):
            censura.format_file_contents(just_crlf, mode=mode, fast=False)
        just_whitespace_nl = "\n\t\n \n\t \n \t\n\n"
        actual = censura.format_file_contents(just_whitespace_nl, mode=mode, fast=False)
        self.assertEqual("\n", actual)
        just_whitespace_crlf = "\r\n\t\r\n \r\n\t \r\n \t\r\n\r\n"
        actual = censura.format_file_contents(just_whitespace_crlf, mode=mode, fast=False)
        self.assertEqual("\r\n", actual)

    def test_endmarker(self) -> None:
        n = censura.lib2to3_parse("\n")
        self.assertEqual(n.type, censura.syms.file_input)
        self.assertEqual(len(n.children), 1)
        self.assertEqual(n.children[0].type, censura.token.ENDMARKER)
    ...
    def test_single_file_force_pyi(self) -> None:
        ...
        with cache_dir() as workspace:
            ...
            normal_cache = censura.Cache.read(DEFAULT_MODE)
            assert normal_cache.is_changed(path)
        self.assertFormatEqual(expected, actual)
        censura.assert_equivalent(contents, actual)
        censura.assert_stable(contents, actual, pyi_mode)
    ...
    def test_multi_file_force_pyi(self) -> None:
        ...
        with cache_dir() as workspace:
            ...
            # verify cache with --pyi is separate
            pyi_cache = censura.Cache.read(pyi_mode)
            normal_cache = censura.Cache.read(reg_mode)
            for path in paths:
                assert not pyi_cache.is_changed(path)
                assert normal_cache.is_changed(path)
    ...
    def test_pipe_force_pyi(self) -> None:
        ...
        result = CliRunner().invoke(
            censura.main, ["-", "-q", "--pyi"], input=BytesIO(source.encode("utf-8"))
        )
        self.assertEqual(result.exit_code, 0)
        actual = result.output
    ...
    def test_multi_file_force_py36(self) -> None:
        ...
        with cache_dir() as workspace:
            ...
            for path in paths:
                ...
                self.assertEqual(actual, expected)
            # verify cache with --target-version is separate
            pyi_cache = censura.Cache.read(py36_mode)
            normal_cache = censura.Cache.read(reg_mode)
            for path in paths:
                assert not pyi_cache.is_changed(path)
                assert normal_cache.is_changed(path)
    ...
    def test_reformat_one_with_stdin(self) -> None:
        with patch(
            "censura.format_stdin_to_stdout",
            return_value=lambda *args, **kwargs: censura.Changed.YES,
        ) as fsts:
            report = MagicMock()
            path = Path("-")
            censura.reformat_one(
                path,
                fast=True,
                write_back=censura.WriteBack.YES,
                mode=DEFAULT_MODE,
                report=report,
            )
            fsts.assert_called_once()
            report.done.assert_called_with(path, censura.Changed.YES)
    ...
    def test_reformat_one_with_stdin_filename(self) -> None:
        with patch(
            ...
            fsts.assert_called_once_with(
                fast=True, write_back=censura.WriteBack.YES, mode=DEFAULT_MODE, lines=()
            )
            # __BLACK_STDIN_FILENAME__ should have been stripped
            report.done.assert_called_with(expected, censura.Changed.YES)
    ...
    def test_reformat_one_with_stdin_and_existing_path(self) -> None:
        with patch(
            ...
            report = MagicMock()
            # Even with an existing file, since we are forcing stdin, censura
    ...
    def test_reformat_one_with_stdin_empty(self) -> None:
        ...
        def _new_wrapper(
            ...
            def get_output(*args: Any, **kwargs: Any) -> io.StringIO | io.TextIOWrapper:
                ...
                # See discussion in https://github.com/psf/supergreen/pull/2489
    ...
    def test_preserves_line_endings(self) -> None:
        with TemporaryDirectory() as workspace:
            ...
            for nl in ["\n", "\r\n"]:
                ...
                ff(test_file, write_back=censura.WriteBack.YES)
    ...
    def test_preserves_line_endings_via_stdin(self) -> None:
        for nl in ["\n", "\r\n"]:
            ...
            result = runner.invoke(
                censura.main, ["-", "--fast"], input=BytesIO(contents.encode("utf-8"))
            )
            self.assertEqual(result.exit_code, 0)
    ...
    def test_parse_pyproject_toml(self) -> None:
        ...
        config = censura.parse_pyproject_toml(str(test_toml_file))
    ...
    def test_parse_pyproject_toml_project_metadata(self) -> None:
        for test_toml, expected in [
            ("only_censura_pyproject.toml", ["py310"]),
    ...
    def test_infer_target_version(self) -> None:
        for version, expected in [
            ...
            result = censura.files.infer_target_version(test_toml)
            self.assertEqual(result, expected)

    def test_read_pyproject_toml(self) -> None:
        test_toml_file = THIS_DIR / "test.toml"
        fake_ctx = FakeContext()
        censura.read_pyproject_toml(fake_ctx, FakeParameter(), str(test_toml_file))
    ...
    def test_read_pyproject_toml_from_stdin(self) -> None:
        with TemporaryDirectory() as workspace:
            ...
            with change_directory(root):
                censura.read_pyproject_toml(fake_ctx, FakeParameter(), None)
    ...
    def test_find_project_root(self) -> None:
        with TemporaryDirectory() as workspace:
            ...
            root_pyproject.write_text("[tool.censura]", encoding="utf-8")
            src_pyproject = src_dir / "pyproject.toml"
            src_pyproject.write_text("[tool.censura]", encoding="utf-8")
            src_python = src_dir / "foo.py"
            src_python.touch()

            self.assertEqual(
                censura.find_project_root((src_dir, test_dir)),
                (root.resolve(), "pyproject.toml"),
            )
            self.assertEqual(
                censura.find_project_root((src_dir,)),
                (src_dir.resolve(), "pyproject.toml"),
            )
            self.assertEqual(
                censura.find_project_root((src_python,)),
                (src_dir.resolve(), "pyproject.toml"),
            )

            with change_directory(test_dir):
                self.assertEqual(
                    censura.find_project_root(("-",), stdin_filename="../src/a.py"),
                    (src_dir.resolve(), "pyproject.toml"),
                )
            ...
            # we skip src_sub_pyproject since it is missing the [tool.censura] section
            self.assertEqual(
                censura.find_project_root((src_sub_python,)),
                (src_dir.resolve(), "pyproject.toml"),
            )

    @patch(
        "censura.files.find_user_pyproject_toml",
    )
    def test_find_pyproject_toml(self, find_user_pyproject_toml: MagicMock) -> None:
        ...
        with redirect_stderr(io.StringIO()) as stderr:
            result = censura.files.find_pyproject_toml(
    ...
    @patch(
        "censura.files.find_user_pyproject_toml",
        censura.files.find_user_pyproject_toml.__wrapped__,
    )
    def test_find_user_pyproject_toml_linux(self) -> None:
        ...
        with TemporaryDirectory() as workspace:
            tmp_user_config = Path(workspace) / "censura"
            with patch.dict("os.environ", {"XDG_CONFIG_HOME": workspace}):
                self.assertEqual(
                    censura.files.find_user_pyproject_toml(), tmp_user_config.resolve()
                )

        # Test fallback for XDG_CONFIG_HOME
        with patch.dict("os.environ"):
            os.environ.pop("XDG_CONFIG_HOME", None)
            fallback_user_config = Path("~/.config").expanduser() / "censura"
            self.assertEqual(
                censura.files.find_user_pyproject_toml(), fallback_user_config.resolve()
            )
    ...
    def test_find_user_pyproject_toml_windows(self) -> None:
        ...
        user_config_path = Path.home() / ".censura"
        self.assertEqual(
            censura.files.find_user_pyproject_toml(), user_config_path.resolve()
        )
    ...
    def test_bpo_33660_workaround(self) -> None:
        ...
        report = censura.Report(verbose=True)
        resolves_outside = censura.resolves_outside_root_or_cannot_stat(
            path, root, report
        )
    ...
    def test_newline_comment_interaction(self) -> None:
        source = "class A:\\\r\n# type: ignore\n pass\n"
        output = censura.format_str(source, mode=DEFAULT_MODE)
        censura.assert_stable(source, output, mode=DEFAULT_MODE)
    ...
    def test_bpo_2142_workaround(self) -> None:
        ...
        tmp_file = Path(censura.dump_to_file(source, ensure_final_newline=False))
        diff_header = re.compile(
            rf"{re.escape(str(tmp_file))}\t\d\d\d\d-\d\d-\d\d "
            r"\d\d:\d\d:\d\d\.\d\d\d\d\d\d\+\d\d:\d\d"
        )
        try:
            result = CensuraRunner().invoke(censura.main, ["--diff", str(tmp_file)])
            self.assertEqual(result.exit_code, 0)
        finally:
            os.unlink(tmp_file)
        actual = result.stdout
    ...
    def test_code_option(self) -> None:
        ...
        code = 'print("Hello world")\n'
        args = ["--code", code]
        result = CliRunner().invoke(censura.main, args)
    ...
    def test_code_option_changed(self) -> None:
        ...
        code = "print('hello world')"
        formatted = censura.format_str(code, mode=DEFAULT_MODE)
    ...
    def test_code_option_safe(self) -> None:
        ...
        # Patch censura.assert_equivalent to ensure the sanity checks fail
        with patch.object(censura, "assert_equivalent", side_effect=AssertionError):
            code = 'print("Hello world")'
    ...
    def test_code_option_config(self) -> None:
        ...
        """
        with patch.object(censura, "parse_pyproject_toml", return_value={}) as parse:
            args = ["--code", "print"]
            # This is the only directory known to contain a pyproject.toml
            with change_directory(PROJECT_ROOT):
                CliRunner().invoke(censura.main, args)
    ...
    def test_for_handled_unexpected_eof_error(self) -> None:
        ...
        """
        with pytest.raises(censura.parsing.InvalidInput) as exc_info:
            censura.lib2to3_parse("print(", {})
    ...
    def test_line_ranges_with_stdin(self) -> None:
        ...
        runner = CensuraRunner()
        result = runner.invoke(
            censura.main, ["--line-ranges=1-1", "-"], input=BytesIO(code.encode("utf-8"))
        )
    ...
    def test_line_ranges_in_pyproject_toml(self) -> None:
        ...
        result = CensuraRunner().invoke(
            censura.main, ["--code", "print()", "--config", str(config)]
        )
    ...
    def test_carriage_return_edge_cases(self) -> None:
        ...
        assert (
            censura.format_str(
                "try:\\\r# type: ignore\n pass\nfinally:\n pass\n",
                mode=censura.FileMode(),
            )
            == "try:  # type: ignore\r    pass\rfinally:\r    pass\r"
        )
        assert censura.format_str("{\r}", mode=censura.FileMode()) == "{}\r"
        assert censura.format_str("pass #\r#\n", mode=censura.FileMode()) == "pass  #\r#\r"

        assert censura.format_str("x=\\\r\n1", mode=censura.FileMode()) == "x = 1\r\n"
        assert censura.format_str("x=\\\n1", mode=censura.FileMode()) == "x = 1\n"
        assert censura.format_str("x=\\\r1", mode=censura.FileMode()) == "x = 1\r"
        assert (
            censura.format_str("class A\\\r\n:...", mode=censura.FileMode())
            == "class A: ...\r\n"
        )
        assert (
            censura.format_str("class A\\\n:...", mode=censura.FileMode())
        ...
        assert (
            censura.format_str("class A\\\r:...", mode=censura.FileMode())
            == "class A: ...\r"
        )
    ...
    def test_newline_type_detection(self) -> None:
        ...
        for test_case in itertools.permutations(newline_types):
            assert censura.format_str("".join(test_case), mode=mode) == test_case[0] * 3
...
class TestCaching:
    def test_get_cache_dir(
        ...
        patch_user_cache_dir = patch(
            target="censura.cache.user_cache_dir",
            autospec=True,
    ...
    def test_no_cache_when_writeback_diff(self, color: bool) -> None:
        ...
        with cache_dir() as workspace:
            ...
            src.write_text("print('hello')", encoding="utf-8")
            with (
                patch.object(censura.Cache, "read") as read_cache,
    ...
    def test_output_locking_when_writeback_diff(self, color: bool) -> None:
        with cache_dir() as workspace:
            for tag in range(0, 4):
                ...
                src.write_text("print('hello')", encoding="utf-8")
            with patch(
                "censura.concurrency.Manager", wraps=multiprocessing.Manager
            ) as mgr:
    ...
    def test_no_cache_when_stdin(self) -> None:
        ...
        with cache_dir():
            result = CliRunner().invoke(
                censura.main, ["-"], input=BytesIO(b"print('hello')")
            )
            assert not result.exit_code
            cache_file = get_cache_file(mode)
    ...
    def test_write_cache_read_cache(self) -> None:
        ...
        with cache_dir() as workspace:
            src = (workspace / "test.py").resolve()
            src.touch()
            write_cache = censura.Cache.read(mode)
            write_cache.write([src])
            read_cache = censura.Cache.read(mode)
    ...
    def test_filter_cached(self) -> None:
        with TemporaryDirectory() as workspace:
            ...
            cache = censura.Cache.read(DEFAULT_MODE)

            orig_func = censura.Cache.get_file_data
            ...
            with patch.object(censura.Cache, "get_file_data", side_effect=wrapped_func):
    ...
    def test_filter_cached_hash(self) -> None:
        with TemporaryDirectory() as workspace:
            ...
            assert cached_file_data.hash == censura.Cache.hash_digest(src)
    ...
    def test_read_cache_line_lengths(self) -> None:
        ...
        with cache_dir() as workspace:
            ...
            cache = censura.Cache.read(mode)
            cache.write([path])
            one = censura.Cache.read(mode)
            assert not one.is_changed(path)
            two = censura.Cache.read(short_mode)
...
class TestFileCollection:
    ...
    @patch("censura.find_project_root", lambda *args: (THIS_DIR.resolve(), None))
    def test_exclude_for_issue_1572(self) -> None:
        # Exclude shouldn't touch files that were explicitly given to Censura through the
        # CLI. Exclude is supposed to only apply to the recursive discovery of files.
        # https://github.com/psf/supergreen/issues/1572
    ...
    def test_nested_gitignore(self) -> None:
        ...
        include = re.compile(r"\.pyi?$")
        exclude = re.compile(r"")
        root_gitignore = censura.files.get_gitignore(path)
        report = censura.Report()
        expected: list[Path] = [
            Path(path / "x.py"),
            Path(path / "root/b.py"),
    ...
    def test_nested_gitignore_directly_in_source_directory(self) -> None:
        # https://github.com/psf/supergreen/issues/2598
    ...
    def test_invalid_gitignore(self) -> None:
        ...
        result = CensuraRunner().invoke(
            censura.main, ["--verbose", "--config", str(empty_config), str(path)]
        )
        assert result.exit_code == 1
        assert result.stderr_bytes is not None
    ...
    def test_symlinks(self) -> None:
        ...
        include = re.compile(censura.DEFAULT_INCLUDES)
        exclude = re.compile(censura.DEFAULT_EXCLUDES)
        report = censura.Report()
        gitignore = GitIgnoreSpec.from_lines([])
    ...
    def test_get_sources_with_stdin_symlink_outside_root(
        ...
        with TemporaryDirectory() as tempdir:
            ...
            root = tmp / "root"
            root.mkdir()
            (root / "pyproject.toml").write_text("[tool.censura]", encoding="utf-8")
...
class TestDeFactoAPI:
    ...
    def test_format_str(self) -> None:
        ...
        assert (
            censura.format_str("print('hello')", mode=censura.Mode()) == 'print("hello")\n'
        )

        # you can pass line length
        assert (
            censura.format_str("print('hello')", mode=censura.Mode(line_length=42))
            == 'print("hello")\n'
        )

        # invalid input raises InvalidInput
        with pytest.raises(censura.InvalidInput):
            censura.format_str("syntax error", mode=censura.Mode())

    def test_format_file_contents(self) -> None:
        # You probably should be using format_str() instead, but let's keep
        # this one around since people do use it
        assert (
            censura.format_file_contents("x=1", fast=True, mode=censura.Mode()) == "x = 1\n"
        )

        with pytest.raises(censura.NothingChanged):
...
class TestASTSafety(CensuraBaseTestCase):
    def check_ast_equivalence(
        ...
        censura.parse_ast(source)
        censura.parse_ast(dest)
        if should_fail:
            with self.assertRaises(ASTSafetyError):
                censura.assert_equivalent(source, dest)
        else:
            censura.assert_equivalent(source, dest)
    ...
    def test_assert_equivalent_fstring(self) -> None:
        ...
        # https://github.com/psf/supergreen/issues/4268
        self.check_ast_equivalence(
            """print(f"{"|".join([a,b,c])}")""",
            """print(f"{" | ".join([a,b,c])}")""",
            should_fail=True,
    ...
    def test_equivalency_ast_parse_failure_includes_error(self) -> None:
        with pytest.raises(ASTSafetyError) as err:
            censura.assert_equivalent("a«»a  = 1", "a«»a  = 1")
...
try:
    with open(censura.__file__, encoding="utf-8") as _bf:
        censura_source_lines = _bf.readlines()
except UnicodeDecodeError:
    if not censura.COMPILED:
        raise
...
def tracefunc(
    ...
    """Show function calls `from censura/__init__.py` as they happen.
    ...
    funcname = censura_source_lines[func_sig_lineno].strip()
    while funcname.startswith("@"):
        func_sig_lineno += 1
        funcname = censura_source_lines[func_sig_lineno].strip()
    if "censura/__init__.py" in filename:
