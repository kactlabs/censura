...
def _initialize_censura_env(upgrade=False):
    if vim.eval("g:censura_use_virtualenv ? 'true' : 'false'") == "false":
        if upgrade:
            print("Upgrade disabled due to g:censura_use_virtualenv being disabled.")
            print(
                "Either use your system package manager (or pip) to upgrade censura"
                " separately,"
            )
            print("or modify your vimrc to have 'let g:censura_use_virtualenv = 1'.")
            return False
        else:
    ...
    if pyver < (3, 10):
        print("Sorry, Censura requires Python 3.10+ to run.")
        return False
    ...
    virtualenv_path = Path(vim.eval("g:censura_virtualenv")).expanduser()
    virtualenv_site_packages = str(
        _get_virtualenv_site_packages(virtualenv_path, pyver)
    )
    first_install = False
    if not virtualenv_path.is_dir():
        print("Please wait, one time setup for Censura.")
        ...
        try:
            ...
            print(
                "(this path can be customized in .vimrc by setting g:censura_virtualenv)"
            )
        ...
        first_install = True
    if first_install:
        print("Installing Censura with pip...")
    if upgrade:
        print("Upgrading Censura with pip...")
    if first_install or upgrade:
        subprocess.run(
            [str(_get_pip(virtualenv_path)), "install", "-U", "censura"],
            stdout=subprocess.PIPE,
        )
        print("DONE! You are all set, thanks for waiting ✨ 🍰 ✨")
    if first_install:
        print(
            "Pro-tip: to upgrade Censura in the future, use the :CensuraUpgrade command and"
            " restart Vim.\n"
        )
...
if _initialize_censura_env():
    try:
        import censura
    except ImportError:
        print(f"Could not import censura from any of: {', '.join(sys.path)}.")
    import time


def get_target_version(tv):
    if isinstance(tv, censura.TargetVersion):
        return tv
    ret = None
    try:
        ret = censura.TargetVersion[tv.upper()]
    except KeyError:
        print(
            f"WARNING: Target version {tv!r} not recognized by Censura, using default"
            " target"
        )
    return ret
...
def Censura(**kwargs):
    ...
    ``target_version`` needs to be cleaned because ``censura.FileMode``
    ...
    consistent to Censura's documentation of the structure of pyproject.toml.
    """
    start = time.time()
    configs = get_configs()
    ...
    if "target_version" in kwargs:
        ...
        censura_kwargs["target_versions"] = target_version
    ...
    try:
        new_buffer_str = censura.format_file_contents(
            buffer_str,
            fast=configs["fast"],
            mode=mode,
        )
    except censura.NothingChanged:
        if not quiet:
            print(
                "Censura: already well formatted, good job. (took"
        ...
        if not quiet:
            print(f"Censura: reformatted in {time.time() - start:.4f}s.")


def get_configs():
    filename = vim.eval("@%")
    path_pyproject_toml = censura.find_pyproject_toml((filename,))
    if path_pyproject_toml:
        toml_config = censura.parse_pyproject_toml(path_pyproject_toml)
    else:
        toml_config = {}
...
def CensuraUpgrade():
    _initialize_censura_env(upgrade=True)


def CensuraVersion():
    print(f"Censura, version {censura.__version__} on Python {sys.version}.")
EndPython3

function censura#Censura(...)
    let kwargs = {}
    for arg in a:000
...
function censura#CensuraUpgrade()
  :py3 CensuraUpgrade()
endfunction

function censura#CensuraVersion()
  :py3 CensuraVersion()
endfunction
