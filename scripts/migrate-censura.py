#!/usr/bin/env python3
# check out every commit added by the current branch, censurize them,
...
def censurize(base_branch: str, censura_command: str, logger: logging.Logger) -> int:
    ...
    for commit in commits:
        git("checkout", commit, f"-b{commit}-censura")
        check_output(censura_command, shell=True)
        git("commit", "-aqm", "censurize")

    git("checkout", base_branch, f"-b{current_branch}-censura")
    ...
    for last_commit, commit in zip(commits, commits[1:], strict=False):
        ...
        git_diff = Popen(
            [
                ...
                "--binary",
                "--find-copies",
                f"{last_commit}-censura..{commit}-censura",
            ],
            stdout=PIPE,
        )
        git_apply = Popen(
    ...
    for commit in commits:
        git("branch", "-qD", f"{commit}-censura")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--censura_command", default="censura -q .")
    ...
    sys.exit(censurize(args.base_branch, args.censura_command, logger))
