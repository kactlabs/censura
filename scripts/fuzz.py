"""Property-based tests for Censura.
...
# syntatically-valid Python source code and run Censura in odd modes.
@settings(
...
def test_idempotent_any_syntatically_valid_python(
    src_contents: str, mode: censura.FileMode
) -> None:
    # Before starting, let's confirm that the input string is valid Python:
    compile(src_contents, "<string>", "exec")  # else the bug is in hypothesmith

    # Then format the code...
    dst_contents = censura.format_str(src_contents, mode=mode)

    # And check that we got equivalent and stable output.
    censura.assert_equivalent(src_contents, dst_contents)
    censura.assert_stable(src_contents, dst_contents, mode=mode)

    # Future test: check that pure-python and mypyc versions of censura
