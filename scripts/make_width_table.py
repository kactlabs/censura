...
narrow (width 1). The table is written to src/censura/_width_table.py (note
that although this file is generated, it is checked into Git) and is used
by the char_width() function in src/censura/strings.py.
...
def main() -> None:
    table_path = join(dirname(__file__), "..", "src", "censura", "_width_table.py")
